...
googleRouter.get('/callback',
    ...
    function(req, res) {
        if(req.user){
            // 사용자의 이름, 나이 정보 유무 확인
            db.query(`select EXISTS (select google_id from user_info where google_id=${req.user.id} AND name=DEFAULT OR age=DEFAULT limit 1) as success`,function (err,result){
