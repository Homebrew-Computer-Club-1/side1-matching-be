...
googleRouter.get('/callback',
    ...
    function(req, res) {
        ...
        db.query(`select EXISTS (SELECT google_id FROM user_info WHERE google_id=? AND (name IS NULL OR age IS NULL OR tel IS NuLL) limit 1) as success`,[req.user?.id],function (err,result){
